  ...
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="优惠券类型" prop="discountType">
        ...
        >
          <el-option
            v-for="dict in this.getDictDatas(DICT_TYPE.PROMOTION_DISCOUNT_TYPE)"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
  ...
  data() {
    return {
      ...
      rules: {
        ...
        discountPercent: [
          { required: true, message: "优惠券折扣不能为空", trigger: "blur" },
        ],
        discountLimitPrice: [
          { required: true, message: "最多优惠不能为空", trigger: "blur" },
        ],
        usePrice: [
          {
            required: true,
            message: "满多少元可以使用不能为空",
            trigger: "blur",
          },
        ],
        takeType: [
        ...
        takeLimitCount: [
          { required: true, message: "每人限领个数不能为空", trigger: "blur" },
        ],
        validityType: [
          { required: true, message: "有效期类型不能为空", trigger: "change" },
        ],
        validTimes: [
          { required: true, message: "固定日期不能为空", trigger: "change" },
        ],
        fixedStartTerm: [
          { required: true, message: "开始领取天数不能为空", trigger: "blur" },
        ],
        fixedEndTerm: [
          { required: true, message: "开始领取天数不能为空", trigger: "blur" },
        ],
    ...
    handleUpdate(row) {
      ...
      getCouponTemplate(id).then((response) => {
        this.form = {
          ...response.data,
          discountPrice:
            response.data.discountPrice !== undefined
              ? response.data.discountPrice / 100.0
              : undefined,
          discountPercent:
            response.data.discountPercent !== undefined
              ? response.data.discountPercent / 10.0
              : undefined,
          discountLimitPrice:
            response.data.discountLimitPrice !== undefined
              ? response.data.discountLimitPrice / 100.0
              : undefined,
          usePrice:
            response.data.usePrice !== undefined
              ? response.data.usePrice / 100.0
              : undefined,
          validTimes: [
            response.data.validStartTime,
            response.data.validEndTime,
          ],
        };
        this.open = true;
        this.title = "修改优惠劵";
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        ...
        let data = {
          ...this.form,
          discountPrice:
            this.form.discountPrice !== undefined
              ? this.form.discountPrice * 100
              : undefined,
          discountPercent:
            this.form.discountPercent !== undefined
              ? this.form.discountPercent * 10
              : undefined,
          discountLimitPrice:
            this.form.discountLimitPrice !== undefined
              ? this.form.discountLimitPrice * 100
              : undefined,
          usePrice:
            this.form.usePrice !== undefined
              ? this.form.usePrice * 100
              : undefined,
          validStartTime:
            this.form.validTimes && this.form.validTimes.length === 2
              ? this.form.validTimes[0]
              : undefined,
          validEndTime:
            this.form.validTimes && this.form.validTimes.length === 2
              ? this.form.validTimes[1]
              : undefined,
        };
        // 修改的提交
