    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="活动状态" align="center" prop="status">
        <template v-slot="scope">
          <dict-tag
            :type="DICT_TYPE.PROMOTION_ACTIVITY_STATUS"
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="商品选择">
          <el-select
            ...
            filterable
            style="width: 880px"
            @change="changeFormSku"
          >
            <el-option
              v-for="item in productSkus"
              :key="item.id"
              :label="item.spuName + ' ' + item.name"
              :value="item.id"
            >
              <span style="float: left"
                >{{ item.spuName }} &nbsp; {{ item.name }}</span
              >
          ...
          <el-row>
            <el-button
              type="primary"
              size="mini"
              @click="batchEditProduct('limitBuyCount')"
              >限购</el-button
            >
            <el-button
              type="primary"
              size="mini"
              @click="batchEditProduct('seckillPrice')"
              >秒杀价</el-button
            >
            <el-button
              type="primary"
              size="mini"
              @click="batchEditProduct('seckillStock')"
              >秒杀库存</el-button
            >
          </el-row>
          <el-table
          ...
          >
            ...
            <el-table-column label="限购(0为不限购)" align="center" width="150">
              <template v-slot="scope">
                <el-input-number
                  v-model="scope.row.limitBuyCount"
                  size="mini"
                  :min="0"
                  :max="10000"
                >
            ...
            >
              <template v-slot="scope">
                <el-button
                  ...
                  type="text"
                  icon="el-icon-delete"
                  @click="removeFormSku(scope.row.skuId)"
                  >删除
                </el-button>
              </template>
            </el-table-column>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        timeIds: [
          { required: true, message: "秒杀场次不能为空", trigger: "blur" },
        ],
        totalPrice: [
          {
            required: true,
            message: "订单实付金额，单位：分不能为空",
            trigger: "blur",
          },
        ],
      },
      ...
      getSeckillActivity(id).then((response) => {
        ...
        this.form.products.forEach((product) => {
          ...
          product.seckillPrice =
            product.seckillPrice !== undefined
              ? product.seckillPrice / 100
              : undefined;
        });
        // 打开弹窗
        this.open = true;
