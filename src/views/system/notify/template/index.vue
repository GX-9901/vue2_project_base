<template>
  <div class="app-container">
    ...
    <el-dialog
      title="发送站内信"
      :visible.sync="sendNotifyOpen"
      width="500px"
      append-to-body
    >
      <el-form
        ref="sendNotifyForm"
        :model="sendNotifyForm"
        :rules="sendNotifyRules"
        label-width="140px"
      >
        <el-form-item label="模板内容" prop="content">
          <el-input
            v-model="sendNotifyForm.content"
            type="textarea"
            placeholder="请输入模板内容"
            readonly
          />
        ...
        >
          <el-input
            v-model="sendNotifyForm.templateParams[param]"
            :placeholder="'请输入 ' + param + ' 参数'"
          />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="submitSendNotifyForm"
          >确 定</el-button
        >
...
export default {
  ...
  methods: {
    ...
    handleSendNotify(row) {
      ...
      this.sendNotifyForm.templateParams = row.params.reduce(function (
        obj,
        item,
      ) {
        obj[item] = undefined;
      ...
      // 根据 row 重置 rules
      this.sendNotifyRules.templateParams = row.params.reduce(function (
        obj,
        item,
      ) {
        obj[item] = {
