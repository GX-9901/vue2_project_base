  ...
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="支付状态" prop="status">
        ...
        >
          <el-option
            v-for="dict in this.getDictDatas(DICT_TYPE.PAY_ORDER_STATUS)"
    ...
    >
      <el-descriptions :column="2" label-class-name="desc-label">
        ...
        <el-descriptions-item label="支付单号">
          <el-tag type="warning" size="small" v-if="orderDetail.no">{{
            orderDetail.no
          }}</el-tag>
        </el-descriptions-item>
      </el-descriptions>
      ...
      <el-descriptions :column="2" label-class-name="desc-label">
        ...
        <el-descriptions-item label="支付金额">
          <el-tag type="success" size="small"
            >￥{{ (orderDetail.price / 100.0).toFixed(2) }}</el-tag
          >
        </el-descriptions-item>
      </el-descriptions>
      <el-descriptions :column="2" label-class-name="desc-label">
        <el-descriptions-item label="手续费">
          <el-tag type="warning" size="small"
            >￥{{ (orderDetail.channelFeePrice / 100.0).toFixed(2) }}</el-tag
          >
        </el-descriptions-item>
        <el-descriptions-item label="手续费比例"
          >{{
            (orderDetail.channelFeeRate / 100.0).toFixed(2)
          }}%</el-descriptions-item
        >
      </el-descriptions>
      <el-descriptions :column="2" label-class-name="desc-label">
      ...
      <el-descriptions :column="2" label-class-name="desc-label">
        ...
        <el-descriptions-item label="更新时间">{{
          parseTime(orderDetail.updateTime)
        }}</el-descriptions-item>
      </el-descriptions>
      <!-- 分割线 -->
      <el-divider />
      ...
      <el-descriptions :column="2" label-class-name="desc-label">
        <el-descriptions-item label="渠道单号">
          <el-tag
            size="mini"
            type="success"
            v-if="orderDetail.channelOrderNo"
            >{{ orderDetail.channelOrderNo }}</el-tag
          >
        </el-descriptions-item>
        <el-descriptions-item label="渠道用户">{{
          orderDetail.channelUserId
        }}</el-descriptions-item>
      </el-descriptions>
      <el-descriptions :column="2" label-class-name="desc-label">
        <el-descriptions-item label="退款金额">
          <el-tag size="mini" type="danger"
            >￥{{ (orderDetail.refundPrice / 100.0).toFixed(2) }}</el-tag
          >
        </el-descriptions-item>
